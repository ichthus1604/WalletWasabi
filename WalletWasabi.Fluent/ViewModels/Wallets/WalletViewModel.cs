...
public partial class WalletViewModel : WalletViewModelBase
{
	...
	protected WalletViewModel(Wallet wallet) : base(wallet)
	{
		...
		ReceiveCommand = ReactiveCommand.Create(() => Navigate(NavigationTarget.DialogScreen).To(new ReceiveViewModel(new WalletModel(wallet), UIContext.Default))); // Temporary measure until refactoring is completed.
